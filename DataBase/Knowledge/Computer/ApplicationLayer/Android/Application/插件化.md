...
1.3 插件化中 有单DexClassLoader和多DexClassLoader两种结构。 插件化要解决的是
### 主工程调用插件
有单DexClassLoader
...
1.3 新建一个subIntent用来启动StubActivity，并将前面得到的TargetActivity的Intent保存到subIntent中，便于 以后还原TargetActivity。
